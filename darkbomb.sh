...
echo -e '''
         ...
         \e[1;91m   ╔╝\e[1;97m███████████████\e[1;91m╚╗
         \e[1;91m   ║\e[1;97m█████\e[1;92mDarkBomb\e[1;97m████\e[1;91m║
...
echo -e "                This is ${S2} DFA BEAST" | pv -qL 150

echo "                 From Dark Hackers !" | pv -qL 150
echo
echo -e "     ${S2}  Ethical Hackers${S7} |${S2} CyberSecurity  Students${S7} |${S2} Programmers" | pv -qL 10
echo
printf "${S2}               our Group ${S7} Dark Hackers${R0}\n" | pv -qL 12
echo
echo -e "       ${S6}          my social handles are below !!" | pv -qL 10
echo

printf "${S2}          Github ${S1}     :    ${S6}https://github.com/DFA-BEAST/${R0}\n" | pv -qL 50
 
echo
printf "${S2}         Instagram ${S1}   :   ${S6}https://instagram.com/dfa_beast/${R0}\n" | pv -qL 50
...
echo 
echo
printf "             ${S2}[${R0} Dark_Bombs${S2}] ${S3}Enter target  >>> ${R0}"
read targett
...
if [ "$options" -eq "2" ];then
  INTERNET
  REQUIREMENTS
  rm -rf $PREFIX/bin/darkbomb > /dev/null 2>&1
  cd $PREFIX/share
  rm -rf darkhack > /dev/null 2>&1
  git clone https://github.com/DFA-BEAST/darkbomb.git
  cd $PREFIX/share/darkbomb && chmod +x darkbomb.sh
  sleep 1
cat <<- CONFI > $PREFIX/bin/darkbomb
#!/bin/bash
args="\$@"
cd \$PREFIX/share/darkbomb && bash darkbomb.sh \$args
CONFI
chmod +x $PREFIX/bin/darkbomb
printf "\n${S2}Just run 'darkbomb' to run this program${R0}\n"
fi
...
if [ "$options" -eq "4" ];then
  echo
  echo -e "${S2}[${S1}~${S2}]${S3} Thanks for Using Dark_Bomb !${S3}"
  echo
  exit 1
else
  menux
...
if [[ $comand == 'darkbomb.sh -u' ||$comand == 'darkbomb.sh --update' ]]; then
  INTERNET
  git pull
  printf "\n${S2}Darkbomb is updated successfully${R0}\n\n"
elif [[ $comand == 'darkbomb.sh -i' || $comand == 'darkbomb.sh --install' ]]; then
  INTERNET
  REQUIREMENTS
  rm -rf $PREFIX/bin/darkbomb > /dev/null 2>&1
  cd $PREFIX/share
  rm -rf darkbomb > /dev/null 2>&1
  git clone https://github.com/DFA-BEAST/darkbomb.git
  cd $PREFIX/share/darkhack && chmod +x darkbomb.sh
  sleep 1
cat <<- CONF > $PREFIX/bin/darkbomb
#!/bin/bash
args="\$@"
cd \$PREFIX/share/darkhack && bash darkbomb.sh \$args
CONF
chmod +x $PREFIX/bin/darkbomb
printf "\n${S2}Just run 'darkbomb' to run this program${R0}\n"
else
